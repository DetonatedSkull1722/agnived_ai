...
FRAME_SKIP = 30              # detect every 10th frame (like test_video.py)
...
class VideoWildlifeDetector:
    ...
    def detect_frame(self, frame_rgb, frame_idx):
        ...
        temp_path = temp_dir / f"frame_{frame_idx}.jpg"
        Image.fromarray(frame_rgb).save(str(temp_path))

        result = self.detector.single_image_detection(str(temp_path))
    ...
    def classify_detection_realtime(self, frame_rgb, bbox):
        ...
        temp_dir = Path(tempfile.gettempdir())
        temp_crop = temp_dir / "classify_crop.jpg"
        cropped_resized.save(str(temp_crop))

        model_predictions = {}
        for model_name, classifier in self.classifiers.items():
            try:
                result = classifier.single_image_classification(str(temp_crop))
...
if __name__ == "__main__":
    ...
    YOUTUBE_LIVE_URL = "https://www.youtube.com/watch?v=EvsLqQS_80E"
